...
namespace Blazorit.Infrastructure.Repositories.Concrete.Identity
{
    public class IdentityRepository : IIdentityRepository {
        ...
        private readonly IDbContextFactory<IdentDbContext> _contextFactory;


        public IdentityRepository(IDbContextFactory<IdentDbContext> contextFactory) {
        ...
        public async Task<UserExistsResult> UserExists(string userName) {            
            try {
                using (var context = _contextFactory.CreateDbContext()) {
                    if (await context.User.AnyAsync(user => user.UserName.ToLower().Equals(userName.ToLower()))) {
        ...
        public async Task<User?> GetUser(string userName) {            
            try {
                using (var context = _contextFactory.CreateDbContext()) {
                    var user = await context.User.FirstOrDefaultAsync(x => x.UserName.ToLower().Equals(userName.ToLower()));
